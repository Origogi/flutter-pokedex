...
final pokemonDetailInfoUseCaseProvider = FutureProvider.autoDispose
    .family<PokemonDetailInfo, int>((ref, id) async {
  final pokemonInfo = await ref.watch(pokemonInfoProvider(id).future);
  final pokemonSpeciesInfo = await ref.watch(pokemonSpeciesInfoProvider(id).future);
  final pokemonWeaknessTypes = await ref.watch(pokemonWeaknessTypesInfoProvider(pokemonInfo.types.first.name).future);
